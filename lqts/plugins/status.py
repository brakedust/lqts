...
async def get_progress(loop, raw=False, long=False, status_filter=None, port=9126, summary=False):
...
@plg_mgr.register
@argh.arg('--raw', action='store_true', help='Display the raw json message returned by the server.')
@argh.arg('--long', action='store_true', help='Display all columns returned from the server.')
...
def status(raw=False, long=False, C=False, Q=False, R=False, port=9126, summarize=False):
