...
async def remove_job(loop, job_id, port=9126, ip_address="127.0.0.1"):

    reader, writer = await asyncio.open_connection(ip_address, port, loop=loop)

    data = {"type": "remove", "job_id": job_id}
...
@argh.arg(
    ...
    "-l",
    type=int,
    help="Last ID to remove.  Specifying this will cause a "
...
def remove(first_id, last_id=None, port=9126, ip_address="127.0.0.1"):
    ...
    if last_id is None:
        loop.run_until_complete(
            remove_job(loop, first_id, port=port, ip_address=ip_address)
        )
        ...
        for job_id in range(first_id, last_id + 1):
            loop.run_until_complete(
                remove_job(loop, job_id, port=port, ip_address=ip_address)
            )
